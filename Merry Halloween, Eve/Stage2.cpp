...
void Stage2::Set()
{
	...
	m_Object[0].set_x(2100);
	m_Object[0].set_y(0);
	m_Object[1].set_x(1700);
	m_Object[1].set_y(-150);
	m_Object[2].set_x(1900);
	m_Object[2].set_y(200);
	m_Object[3].set_x(1850);
	m_Object[3].set_y(-250);
	m_Object[4].set_x(2000);
	m_Object[4].set_y(50);
...
void Stage2::Camera_Move_X(int num)
{
	//여기서 1200은 클라이언트 가로
...
void Stage2::Camera_Move_Y(int num)
{
	//여기서 800은 클라이언트 세로
	//예외처리
...
void Stage2::Mouse_Down(LPARAM lParam)
{
	if (m_bView) return;
	if (m_bThrow) return;
...
void Stage2::Mouse_Move(LPARAM lParam)
{
...
void Stage2::Keyboard_Down(WPARAM wParam)
{
	switch (wParam)
	{
	case VK_SPACE:
